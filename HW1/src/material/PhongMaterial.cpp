...
VertexOutput PhongMaterial::vertexShader(const Vertex& vertex, const Color& frontColor, const Color& backColor, const UniformContext& ctx) const {
	VertexOutput out;
...
Color PhongMaterial::fragmentShader(const Fragment& fragment, const UniformContext& ctx) const {
	...
	for (const Light& light : ctx.scene.lights) {
		...
		const Vector3 l = Normalize(light.position - fragment.varyings.worldPosition);
